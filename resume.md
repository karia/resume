...
- **チーム規模**: 開発チーム全体：20名, インフラチーム：4名 (正社員1名 + 業務委託3名)
...
サービスの安定性・信頼性向上（アラートの削減）のためにインフラ構成の最適化に取り組みました。また低コストなインフラ運用の徹底や、CI/CDパイプラインの改善、セキュリティ対策にも注力しました。開発チーム全体に関わるプロセス改善にも積極的に取り組み、インフラチーム増員後はチームをリードする立場も担いました。
...
- Ruby on RailsアプリケーションのDocker化

##### 自動化・コード化

- ECS on EC2構成のFargate移行
- GitHub ActionsでのCI/CDパイプライン設計・実装（Docker Buildプロセス自動化、ECSデプロイ自動化等）
- CircleCIのジョブ分割等によるハードウェアリソース最適化
- Terraformによる既存AWSインフラのIaC化
